...
@section('panel')
    <div class="card">
        <form class="card-body rtl" method="post" action="{{ route('admin.roles.update', $role) }}">
            @method('put')
            @csrf
            <div class="row">
                    ...
                    <div class="form-group">
                        ...
                        <select name="permissions[]" multiple class="form-control">
                            @foreach(\App\Enums\Permissions::all() as $p)
                                <option {{ in_array($p, $role->permissions()) ? 'selected' : '' }}
                                        value="{{ $p }}">{{ trans('words.permissions.'. $p) }}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
