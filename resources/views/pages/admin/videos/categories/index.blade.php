...
@section('title', 'دسته‌بندی‌ ویدئوها')
    ...
    <div class="card">
        <div class="card-body rtl">
            <div class="form-group">
                <a href="{{ route('admin.videos.categories.create') }}" class="btn btn-primary">افزودن دسته</a>
            </div>
                ...
                <table class="table table-striped">
                    ...
                    @foreach($categories as $category)
                        <tr>
                            ...
                            <td>{{ $category->parent_id != 0 ? $category->parent->title : 'بدون والد' }}</td>
                            <td>{{$category->position != null ? $category->position : 'مشخص نشده'}}</td>
                            <td>
                                <a href="{{ route('admin.videos.categories.edit', $category) }}"
                                   class="btn btn-sm btn-info">ویرایش</a>
                                <form class="form-inline d-inline-block" method="post"
                                      action="{{ route('admin.videos.categories.destroy', $category) }}">
                                    @csrf
                                    @method('delete')
